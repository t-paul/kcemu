...
int
RomRegistryEntry::get_size() const
{
  return _size;
}
...
bool
RomRegistry::save_rom(const RomRegistryEntry *entry, byte_t *buf, unsigned int size) const
{
  struct stat statbuf;
  string homedir(kcemu_homedir);
  string romdir = homedir + "/roms";

  if (stat(romdir.c_str(), &statbuf) != 0)
    if (sys_mkdir(romdir.c_str(), 0700) != 0)
      return false;

  string rompath = romdir + "/" + entry->get_id();

  FILE *f = fopen(rompath.c_str(), "wb");
  if (f == NULL)
    return false;

  bool ret = fwrite(buf, 1, size, f) == size;
  if (fsync(fileno(f)) != 0)
    ret = false;
  if (fclose(f) != 0)
    ret = false;
...
bool
RomRegistry::is_missing(const RomRegistryEntry *entry) const
{
  string datadir(kcemu_datadir);
  string rompath = datadir + entry->get_directory() + "/" + entry->get_filename();

  if (access(rompath.c_str(), R_OK) == 0)
    return false;

  string homedir(kcemu_homedir);
  string localrompath = homedir + "/roms/" + entry->get_id();
  return access(localrompath.c_str(), R_OK) != 0;
}

const list<const RomRegistryEntry *> *
RomRegistry::get_missing(const string directory) const
{
  list<const RomRegistryEntry *> *missing_roms = new list<const RomRegistryEntry *>();
  
  for (rom_entry_map_t::const_iterator it = _entries.begin();it != _entries.end();it++)
    {
      const char *dir = (*it).second->get_directory();
      if (!directory.empty() && (directory != dir))
        continue;

      if (is_missing((*it).second))
        {
          missing_roms->push_back((*it).second);
        }
    }
...
int
RomRegistry::count_missing_roms(void) const
{
  return count_missing_roms("");
}

int
RomRegistry::count_missing_roms(const string directory) const
{
  const list<const RomRegistryEntry *> *missing_roms = get_missing(directory);
  int missing = missing_roms->size();
  delete missing_roms;
  return missing;
}
