...
GIDE::GIDE(void)
{
  ...
  _error = 0;
  _status = GIDE_STATUS_READY | GIDE_STATUS_SEEK_COMPLETE | GIDE_STATUS_DATA_REQUEST;

  //_in = fopen("/home/tp/projects/KCemu/DISK/microdos.dump", "rb");
  _in = fopen("/tmp/gide.bin", "rb");
  /*
  if (_in != NULL)
    {
      fgetc(_in);
...
GIDE::in_data(void)
{
  ...
  if (x == 512)
    _status &= ~GIDE_STATUS_DATA_REQUEST;
  ...
  switch (val)
    ...
    case 0x20:
      DBG(2, form("KCemu/GIDE/out_command",
		  "GIDE::out(): e1h: READ SECTORS WITH RETRY\n"));
      break;
    case 0x90:
      DBG(2, form("KCemu/GIDE/out_command",
    ...
    case 0xe1:
      DBG(2, form("KCemu/GIDE/out_command",
		  "GIDE::out(): e1h: SET IDLE MODE\n"));
      break;
    case 0xec:
      DBG(2, form("KCemu/GIDE/out_command",
		  "GIDE::out(): ech: IDENTIFY DRIVE\n"));
      break;
    }
}
